...
4. Due to the overhead of collecting documents and setting up a separate vector database, the document corpus is mocked and a barebones version of the document retrieval system was instrumented. In a more production environment, the RAG system would have been developed differently, as seen above in [pointer 6](#plan-for-scaling-to-production).
5. Last point to note would be that no code was auto accepted from prompting, and every line of code that was generated was manually checked to prevent hallucinations and poor code quality.